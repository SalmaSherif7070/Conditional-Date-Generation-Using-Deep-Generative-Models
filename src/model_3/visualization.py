"""
src/model_3/visualization.py
Three plots: ELBO loss curves, loss log scale, per-condition accuracy bar chart.
"""
import os
import matplotlib
matplotlib.use("Agg")
...
def plot_loss_curves(history: dict, save_dir: str):
    with plt.rc_context(STYLE):
        ...
        ax.plot(history["epochs"], history["train_loss"], lw=2, color=C_TR,  label="Train ELBO")
        ax.plot(history["epochs"], history["val_loss"],   lw=2, color=C_VAL, label="Val ELBO", ls="--")
        ax.set_xlabel("Epoch"); ax.set_ylabel("ELBO Loss")
        ax.set_title("Model 3 – ELBO Loss (Train vs Val)"); ax.legend(); ax.grid(alpha=0.3)
...
def plot_loss_log_scale(history: dict, save_dir: str):
    with plt.rc_context(STYLE):
        ...
        ax.set_title("Model 3 – Loss Log Scale"); ax.legend(); ax.grid(alpha=0.3, which="both")
...
def plot_condition_breakdown(metrics: dict, save_dir: str):
    ...
    with plt.rc_context(STYLE):
        ...
        ax.set_ylabel("Accuracy"); ax.set_title("Model 3 – Per-Condition Accuracy")
        ax.set_ylim(0, 1.15)
